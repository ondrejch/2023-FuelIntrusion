...
def s2comp(h5file:str="") -> str:
    ...
    for i in range(1, comp.len()):
        ...
        for iso in isos:            # Loop over isotopes in mixture and make Serpent-like entries
            if iso[1] <= 0.0:
                continue
            if int(iso[0]) > 3006000 or (int(iso[0]) >=6000 and int(iso[0]) < 6099): # Special treatment for graphite
...
for intrusion in [0,1.0]:
    ...
    for T in np.linspace(300, 990, 70):
        ...
        s2matdeck:str = s2comp(f'../../../07-SHIFT-UThC-noSab/{deckpath1}/{deckpath2}/uglat.shift-output.h5')
        fout = open('lats2','w')
        fout.write(f'''
