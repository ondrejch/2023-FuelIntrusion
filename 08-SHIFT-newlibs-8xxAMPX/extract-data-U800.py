...
fout = open("u8-kdata-saltintrusion.dat","w")
dout = open("u8-datafit-saltintrusion.dat","w")
...
for intrusion in [0,1.0]:
    ...
    for T in np.linspace(800, 890, 10):
...
#    continue
    ...
    ax.set_title(f"SCALE, uranium dust intrusion {intrusion:-5.02f}%", fontsize=20)
    ...
    leg = ax.legend()
    #plt.show()
    fig.savefig(f"u800-intrfit_{intrusion:05.02f}.png", bbox_inches="tight", facecolor='white')
    plt.close()
